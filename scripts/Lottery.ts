...
async function initContracts() {
    ...
    const token = tokenFactory.attach(tokenAddress) as LotteryToken;
}
...
async function openBets(duration: string) {
    const currentBlock = await ethers.provider.getBlock("latest"); const timestamp = currentBlock?.timestamp ?? 0;
    const tx = await contract.openBets(timestamp + Number(duration)); const receipt = await tx.wait(); console.log(`Bets opened (${receipt?.hash})`);
}
...
async function buyTokens(index: string, amount: string) {
    const tx = await contract
        ...
        .purchaseTokens({ value: ethers.parseUnits(amount) / TOKEN_RATIO });
